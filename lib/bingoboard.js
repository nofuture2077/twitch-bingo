...
function buildUserIndex(length, seed) {
    let mapping = [...Array(length).keys()];
    return shuffleSeed.shuffle(mapping, seed);
}

function randomSample(items, length, seed) {
    var shuffledMapping = buildUserIndex(length, seed);
    let result = [];

    for (i = 0; i < length; i++) {
        result.push(items.at(shuffledMapping[i]));
    }
    return result;
}
